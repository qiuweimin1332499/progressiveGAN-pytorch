...
def deconv(layers,c_in,c_out,k_size,stride=1,pad=0,
           leaky=True,bn=False,wn=False,pixel=False,only=False):
...
def conv(layers,c_in,c_out,k_size,stride=1,pad=0,
         leaky=True,bn=False,wn=False,pixel=False,gdrop=True,only=False):
...
class Generator(nn.Module):
    ...
    def first_block(self):
        ...
        layers=deconv(layers,self.nz,ndim,4,1,3,
                      self.flag_leaky,self.flag_bn,self.flag_wn,self.flag_pixelwise)
    ...
    def to_rgb_block(self,c_in):
        ...
        layers=deconv(layers,c_in,self.nc,1,1,0,
                      self.flag_leaky,self.flag_bn,self.flag_wn,self.flag_pixelwise,only=True)
    ...
    def grow_network(self,resl):
        ...
        if resl>=3 and resl<=9:
            print('growing network[{}x{} to {}x{}].'
                  'it may take few seconds...'.format(int(pow(2,resl-1)),
                                                      int(pow(2,resl-1)),
                                                      int(pow(2,resl)),
                                                      int(pow(2,resl))))
    ...
    def freeze_layers(self):    # let's freeze pretrained blocks. (Found freezing layers not helpful)
        print('freeze pretrained weights...')
        for param in self.model.parameters():
            param.requires_grad=False

    def forward(self,x):
        x=self.model(x.view(x.size(0),-1,1,1))  #从1开始逐步反卷积
        return x
...
class Discriminator(nn.Module):
    ...
    def last_block(self):
        ndim = self.ndf
        layers = []
        layers.append(minibatch_std_concat_layer())  # 加入批量标准化层 minibatch_std_concat_layer
        layers = conv(layers, ndim + 1, ndim, 3, 1, 1,
                      self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
        layers = conv(layers, ndim, ndim, 4, 1, 0,
                      self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
    ...
    def intermediate_block(self, resl):
        ...
        layers = []
        if halving:
            layers = conv(layers, ndim, ndim, 3, 1, 1,
                          self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
            layers = conv(layers, ndim, ndim * 2, 3, 1, 1,
                          self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
        else:
            layers = conv(layers, ndim, ndim, 3, 1, 1,
                          self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
    ...
    def from_rgb_block(self, ndim):
        layers = []
        layers = conv(layers, self.nc, ndim, 1, 1, 0,
                      self.flag_leaky, self.flag_bn, self.flag_wn, pixel=False)
        return nn.Sequential(*layers)
    ...
    def grow_network(self, resl):
        if resl >= 3 and resl <= 9:
            ...
            prev_block = nn.Sequential()
            prev_block.add_module('loe_resl_downsample', nn.AvgPool2d(kernel_size=2))
