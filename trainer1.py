    ...
    def train(self):
        ...
        for step in range(2,self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK,self.loader.batch_size)):
                ...
                #print(self.fx_tilde.squeeze().shape)
                #print(self.real_label.shape)
                #print(self.fake_label.shape)
                ...
                log_msg = ' [E:{0}][T:{1}][{2:6}/{3:6}]  errD: {4:.4f} | errG: {5:.4f} | ' \
                    ...
                    self.epoch, self.globalTick, self.stack, len(self.loader.dataset), loss_d.item(),
                    loss_g.item(), self.resl, int(pow(2, floor(self.resl))), self.phase, self.complete['gen'],
                    self.complete['dis'],self.lr)
                tqdm.write(log_msg)
