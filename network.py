...
def deconv(layers, c_in, c_out, k_size, stride=1, pad=0, leaky=True, bn=False, wn=False, pixel=False, only=False):
    if wn:
        layers.append(equalized_deconv2d(c_in, c_out, k_size, stride, pad))
    else:
...
def deepcopy_module(module, target):
    ...
    for name, m in module.named_children():
        if name == target:
            ...
            new_module[-1].load_state_dict(m.state_dict())  # copy weights and structure of the target module
    return new_module
...
class Generator(nn.Module):
    ...
    def first_block(self):
        ...
        if self.flag_norm_latent:
            layers.append(pixelwise_norm_layer())
        layers = deconv(layers, self.nz, ndim, 4, 1, 0, self.flag_leaky, self.flag_bn, self.flag_wn,self.flag_pixelwise)  # 卷积核参数修改保证输入输出大小合适
    ...
    def forward(self, x):
        x = self.model(x.view(x.size(0), -1, 1, 1))  # input as noise vector deconv的输入不能是宽或高为1！！！
        return x
...
class Discriminator(nn.Module):
    ...
    def last_block(self):
        ...
        ndim = self.ndf
        layers = []
        layers.append(minibatch_std_concat_layer())  # minibatch_std gains one channel
    ...
    def grow_network(self, resl):
        if resl >= 3 and resl <= 9:
            ...
            new_model = nn.Sequential()
            new_model.add_module('concat_block', ConcatTable(prev_block, next_block))
            new_model.add_module('fadein_block', fadein_layer(self.config))   #相当于执行一次带权重求和运算
            # we make new network since pytorch does not support remove_module()
            names = get_module_names(self.model)
            for name, module in self.model.named_children():
