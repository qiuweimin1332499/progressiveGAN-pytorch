...
#general settings
parser.add_argument('--train_data_root',type=str,default=".data/CelebA")
parser.add_argument('--random_seed',type=int,default=int(time.time()))
parser.add_argument('--n_gpu',type=int,default=1)     #for multi-GPU

#training parameters
parser.add_argument('--lr',type=float,default=0.001)    #learning rate
parser.add_argument('--lr_decay',type=float,default=0.87)     #learning rate decay at every resolution
parser.add_argument('--eps_drift',type=float,default=0.001)     #coeff for the drift loss
parser.add_argument('--smoothing',type=float,default=0.997)     #soomthing factor for smoothed generator
...
parser.add_argument('--ngf',type=int,default=512)     #feature dimension of final layer of noise
parser.add_argument('--ndf',type=int,default=512)      #feature dimension of first layer of discriminator
parser.add_argument('--TICK',type=int,default=100)      # 1 TICK=1000 images=(1000/batch_size)iter即每训练TICK张图片计为迭代一次
...
parser.add_argument('--stab_tick',type=int,default=100)      #stabiliztion tick

#network structure
parser.add_argument('--flag_wn',type=bool,default=False)     #use of equalized-learning rate
parser.add_argument('--flag_bn',type=bool,default=False)     #use of batch-normalization
...
parser.add_argument('--flag_add_noise',type=bool,default=True)     #add noise to the real image
parser.add_argument('--flag_norm_latent',type=bool,default=True)    #pixelwise normalization of latent vector
